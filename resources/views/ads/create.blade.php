    ...
    <div class="mt-2">
        ...
        <div class="form-group position-relative">
			...
			<select class="form-control" name="age" id="age">
				@for ($i=18; $i < 51; $i++)
					<option value="{{ $i }}" @if (old('age') == $i) selected @endif >
						{{ $i }}</option>
				@endfor
			</select>
...
<div class="col-md-12 mobile-mb-2">
  ...
  <input type="hidden" id="adsdesciption" value="1" name="adsdesciption">
    ...
    <textarea name="form[post_content]"  class="form-control rounded-0 required-field" tabindex="2" data-key="field-post_content" id="field-post_content">{{ old('adsdesciption') }}</textarea>
  </div>
...
function fetchQuarters(town){
	...
	@if (old('town') != "")
		fetchQuarters(document.getElementById('town'))
	@endif
...
function processSubmitForm(){
	...
	if(jQuery('#quarter').val() === "" || jQuery('#quarter').val() === undefined){
    	...
    	if(jQuery('#field-post_content').val() === "" || jQuery('#field-post_content').val() === undefined){
		 ...
		 //console.log(jQuery('#adsdesciption').val());
